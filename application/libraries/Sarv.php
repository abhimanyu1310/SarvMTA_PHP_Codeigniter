...
class Sarv {
    ...
    public function init($owner_id = null, $apikey = null, $SarvTES_APP_DOMAIN = null) {
        if (!$apikey)
            throw new Sarv_Error('You must provide a Sarv Token');
        if (!$owner_id)
            throw new Sarv_Error('You must provide a Sarv Owner id');
        if (!$SarvTES_APP_DOMAIN)
            throw new Sarv_Error('You must provide a Sarv TES DOMAIN');
        $this->apikey = $apikey;
    ...
    public function call($url, $params) {
        if (!isset($this->ch)) {
            throw new Sarv_Error('You must call init method first');
        }
        ...
        if ($result === null)
            throw new Sarv_Error('We were unable to decode the JSON response from the Sarv API: ' . $response_body);
    ...
    public function castError($result) {
        if ($result['status'] !== 'error' || !$result['name'])
            throw new Sarv_Error('We received an unexpected error: ' . json_encode($result));
